...
def run_ebita_and_tax_agent(
    ...
    sys_prompt = (
        ...
        "     - If an adjustment is positive (increases EBITA), its associated tax adjustment (tax effect at 25% statutory rate) must also be positive, which increases the Adjusted Taxes (making the tax provision less negative/closer to zero).\n"
        "     - If an adjustment is negative (decreases EBITA), its associated tax adjustment must also be negative, which decreases Adjusted Taxes (making it more negative).\n"
        "     - The adjustments to EBITA and their associated tax adjustments must always move in the same direction (have the same sign) because the statutory tax rate of 25% is positive.\n"
