...
def calculate_modeling(ticker, md_path):
    ...
    profile_cmd = [
        sys.executable,
        os.path.join(os.path.dirname(__file__), "market_data.py"),
        "profile",
        ticker,
    ]
    ...
    for line in meta_lines:
        if line.startswith("# "):
            ...
            if "Unit" in line:
                ...
                m = re.search(
                    r"\|\s*ADR Ratio\s*\|\s*([\d\.]+)\s*\|", line, re.IGNORECASE
                )
                if m:
                    adr_ratio = float(m.group(1).strip())
        ...
        if fx_res.returncode == 0:
            try:
                ...
                if fx_data.get("valid"):
                    ...
                    print(
                        f"  Fetched FX Rate: 1 {local_currency} ({query_curr}) = {fx_rate} USD"
                    )
                else:
    ...
    l4q_growth = (
        sum(clean_value(q.get("Organic Growth")) for q in l4q) / len(l4q) if l4q else 0
    )
    l4q_tax = (
        sum(clean_value(q.get("Adj Tax Rate")) for q in l4q) / len(l4q) if l4q else 0.21
    )
    ...
    if doc_table:
        ...
        if doc_file:
            ...
            if os.path.exists(doc_path):
                ...
                reported_shares = clean_value(
                    fin_summary.get("Diluted Shares Outstanding", "0")
                )
                if reported_shares > 0:
                    shares_out = (reported_shares * unit_factor) / adr_ratio
                    print(
                        f"  Using reported shares ({reported_shares} {unit_label}) / ADR ratio {adr_ratio} -> {shares_out:,.0f} pricing shares"
                    )

                print(
                    f"  Balance sheet data from {doc_file.group(1)}: cash={cash}, debt={debt}, interest_ann={interest}"
                )
    ...
    w_d = 1.0 - w_e
    wacc_raw = (
        w_e * cost_equity + w_d * cost_debt * (1 - tax_stat)
        if debt > 0
        else cost_equity
    )
    ...
    if avg_turnover < 0 or avg_turnover > 100:
        ...
        if avg_turnover == 0:
            mct = 100.0
            mct_rationale = "Defaulted to 100.0x because historical average is 0."
        else:
...
"""
    ...
    proj_headers = (
        "| | Base | "
        + " | ".join([f"Yr {p['year']}" for p in projections])
        + " | Terminal |"
    )
    proj_sep = "|---|------|" + "|".join(["---"] * 10) + "|----------|"
    ...
    ic_row = (
        ...
        + " | ".join([f"{p['ic']:,.0f}" for p in projections])
        + " | -- |"
    )
    roic_row = (
    ...
    fcf_row = (
        "| FCF | -- | "
        + " | ".join([f"{p['fcf']:,.0f}" for p in projections])
        + f" | {tv_fcf:,.0f} |"
    )
    pv_row = (
        "| PV of FCF | -- | "
        + " | ".join([f"{p['pv']:,.0f}" for p in projections])
        + f" | {pv_tv:,.0f} |"
    )
...
"""
    ...
    new_content = replace_section(
        new_content, "## DCF Assumptions", assumptions_section
    )
