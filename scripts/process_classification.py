...
from market_data import validate_ticker  # noqa: E402


def main():
    parser = argparse.ArgumentParser(
        description="Process document classification and orchestration."
    )
    ...
    if not val_result["valid"]:
        print(
            f"Error: Ticker {args.ticker} is invalid according to Yahoo Finance. {val_result.get('error')}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.document_type not in doc_types:
        print(
            f"Error: Unknown document type {args.document_type}. Valid types: {list(doc_types.keys())}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.document_type == "earnings_announcement" and final_time_period.startswith(
        "FY"
    ):
    ...
    if not os.path.exists(input_path):
        ...
        if not os.path.exists(processing_path_orig):
            print(
                f"Error: Could not find {args.filename} in input_data/", file=sys.stderr
            )
            sys.exit(1)
