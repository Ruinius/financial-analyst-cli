...
def validate_ticker(ticker: str, verbose: bool = False) -> dict:
    ...
    try:
        ...
        if verbose:
            print(
                f"[DEBUG] Yahoo Finance info keys: {list(info.keys())[:10]}...",
                file=sys.stderr,
            )
...
def get_market_profile(ticker: str, verbose: bool = False) -> dict:
    ...
    try:
        ...
        if verbose:
            print(
                f"[DEBUG] {company_name}: price={result['share_price']}, "
...
def get_exchange_rate(
    from_currency: str, to_currency: str = "USD", verbose: bool = False
) -> dict:
    """
    ...
    try:
        ...
        if price is not None and price > 0:
            ...
            if verbose:
                print(
                    f"[DEBUG] {from_curr}/{to_curr} = {result['rate']}", file=sys.stderr
                )
        else:
...
Examples:
  ...
  python market_data.py fx RMB USD
    ...
    p_fx.add_argument(
        "to_currency", nargs="?", default="USD", help="Target currency (default: USD)"
    )
    ...
    if args.command == "validate":
        ...
        result = get_exchange_rate(
            args.from_currency, args.to_currency, verbose=args.verbose
        )
        print(json.dumps(result, indent=2))
