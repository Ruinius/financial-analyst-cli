...
def section_exists(content, header):
    ...
    if header in content:
        print(
            f"SKIP: '{header}' already exists in output file. Remove it manually to re-extract."
        )
        return True
    return False
...
def req_transformer(path, items):
    ...
    req = urllib.request.Request(
        f"http://localhost:8000/predict/{path}",
        method="POST",
...
def main():
    parser = argparse.ArgumentParser(
        description="Transform and append extracted financial data."
    )
    parser.add_argument(
        "--json", required=True, help="Path to input extracted data JSON"
    )
    parser.add_argument(
        "--md", required=True, help="Path to the output markdown file to append to"
    )
    ...
    # Balance Sheet
    if "balance_sheet" in data and not section_exists(
        existing_content, "## Balance Sheet"
    ):
    ...
    # Income Statement
    if "income_statement" in data and not section_exists(
        existing_content, "## Income Statement"
    ):
        ...
        out_md.append("| --- | --- |")
        out_md.append(
            f"| Basic Shares Outstanding | {is_data.get('basic_shares', 'N/A')} |"
        )
        out_md.append(f"| Basic Unit | {is_data.get('shares_unit', 'millions')} |")
        out_md.append(
            f"| Diluted Shares Outstanding | {is_data.get('diluted_shares', 'N/A')} |"
        )
        out_md.append(f"| Diluted Unit | {is_data.get('shares_unit', 'millions')} |")
        out_md.append(f"| Extraction Date | {today} |")

    # Organic Growth
    if "organic_growth" in data and not section_exists(
        existing_content, "## Organic Growth"
    ):
        og = data["organic_growth"]
        curr_rev = og.get("current_revenue", 0)
        prior_rev = og.get("prior_revenue", 0)
        simple_growth = (
            round(((curr_rev - prior_rev) / prior_rev) * 100, 2) if prior_rev else "N/A"
        )
        ...
        out_md.append(
            f"| Organic Growth (%) | {org_growth if org_growth is not None else 'Not reported'} |"
        )
        out_md.append(f"| **Final Growth (%)** | **{final_growth}** |")
        out_md.append(
            f"| Growth Source | {'Reported constant-currency' if org_growth is not None else 'Calculated simple YoY'} |"
        )
        out_md.append(f"| Extraction Date | {today} |")

    # GAAP Reconciliation
    if "gaap_reconciliation" in data and not section_exists(
        existing_content, "## GAAP Reconciliation"
    ):
        ...
        out_md.append("| --- | --- |")
        out_md.append(
            f"| Reconciliation Type | {gaap.get('reconciliation_type', 'Operating Income')} |"
        )
